<!-- BEGIN BASE-->
	<div id="base">
		...
		<div class="section-header">
			<h2 class="text-primary">Tạo Fact</h2>
		</div>
		<div class="section-body">
			...
			if (isset($_POST['submit'])){
				if ($_SESSION['role'] == 1) {
					$condition = 'icon, title, count';
					$value = ':icon, :title, :count';
					$data = array(
							':icon' => RemoveHack($_POST['icon']),
							':title' => RemoveHack($_POST['title']),
							':count' => RemoveHack($_POST['count'])
						);
					$id = Fact_Model::create($condition,$value,$data);
					if (!empty($id)) {
						header('Location: '.Config::SITE_URL.'/admincp/?action=fact&mode=edit&id='.$id);
					}else{
							...
							<div class="card-body">
								<form class="form-horizontal" role="form" method="post">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<textarea name="count" class="form-control" rows="2" placeholder=""></textarea><div class="form-control-line"></div>
										</div>
									</div>
