<!-- BEGIN BASE-->
	<div id="base">
		...
		<div class="section-header">
			<h2 class="text-primary">Tạo Feature</h2>
		</div>
		<div class="section-body">
			...
			if (isset($_POST['submit'])){
				if ($_SESSION['role'] == 1) {
					$condition = 'icon, title, entry';
					$value = ':icon, :title, :entry';
					$data = array(
							':icon' => RemoveHack($_POST['icon']),
							':title' => RemoveHack($_POST['title']),
							':entry' => RemoveHack($_POST['entry'])
						);
					$id = Feature_Model::create($condition,$value,$data);
					if (!empty($id)) {
						header('Location: '.Config::SITE_URL.'/admincp/?action=feature&mode=edit&id='.$id);
					}else{
							...
							<div class="card-body">
								<form class="form-horizontal" role="form" method="post">
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" name="icon"><div class="form-control-line"></div>
										</div>
									</div>
									<div class="form-group">
										<label for="title" class="col-sm-2 control-label">Title</label>
										<div class="col-sm-10">
											<input type="text" class="form-control" name="title"><div class="form-control-line"></div>
										</div>
									</div>
									<div class="form-group">
										<label for="entry" class="col-sm-2 control-label">Entry</label>
										<div class="col-sm-10">
											<textarea name="entry" class="form-control" rows="2" placeholder=""></textarea><div class="form-control-line"></div>
										</div>
									</div>
