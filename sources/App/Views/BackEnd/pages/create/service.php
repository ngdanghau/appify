<!-- BEGIN BASE-->
	<div id="base">
		...
		<div class="section-header">
			<h2 class="text-primary">Tạo Service</h2>
		</div>
		<div class="section-body">
			...
			if (isset($_POST['submit'])){
				if ($_SESSION['role'] == 1) {
					$condition = 'icon, title, subtext';
					$value = ':icon, :title, :subtext';
					$data = array(
							':icon' => RemoveHack($_POST['icon']),
							':title' => RemoveHack($_POST['title']),
							':subtext' => RemoveHack($_POST['subtext'])
						);
					$id = Service_Model::create($condition,$value,$data);
					if (!empty($id)) {
						header('Location: '.Config::SITE_URL.'/admincp/?action=service&mode=edit&id='.$id);
					}else{
							...
							<div class="card-body">
								<form class="form-horizontal" role="form" method="post">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<textarea name="subtext" class="form-control" rows="2" placeholder=""></textarea><div class="form-control-line"></div>
										</div>
									</div>
