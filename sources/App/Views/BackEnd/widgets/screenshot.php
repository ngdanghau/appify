<!-- BEGIN BASE-->
	<div id="base">
		...
		<div class="section-header">
			<h2 class="text-primary">Cài Đặt Widget Screenshot</h2>
		</div>
...
">Vào Đây</a>
			...
			if (isset($_POST['submit'])){
				...
				foreach ($arr as $item) {
					...
					if (Widget_Screenshot::update($condition,$data)) {
						header('Location: '.Config::SITE_URL.'/admincp/?action=screenshot&mode=config');
					}else{
				...
				<div class="card-body">
					<form class="form" role="form" method="post">
						<div class="form-group floating-label">
							<input type="text" class="form-control" name="icon" value="<?php echo Widget_Screenshot::get('icon');?>">
							<label for="icon">Icon</label>
						</div>	
						<div class="form-group floating-label">
							<input type="text" class="form-control" name="title" value="<?php echo Widget_Screenshot::get('title');?>">
							<label for="title">Title</label>
						</div>	
						<div class="form-group floating-label">
							<textarea name="subtext" class="form-control" rows="2" placeholder=""><?php echo Widget_Screenshot::get('subtext');?></textarea>
